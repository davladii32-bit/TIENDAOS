...
app.get('/api/ventas', authMiddleware, (req, res) => {
  ...
  const params = [];
  if (desde) {
    // Restar 6h para cubrir ventas guardadas en UTC (antes del fix de timezone)
    query += " AND v.creado_en >= datetime(?, '-6 hours')";
    params.push(desde);
  }
  if (hasta) {
    query += " AND v.creado_en <= datetime(?, '+18 hours')";
    params.push(hasta);
  }
...
app.delete('/api/ventas/:id', authMiddleware, (req, res) => {
  ...
  const cancelarVenta = db.transaction(() => {
    // Restaurar stock de cada producto
    const items = db.prepare('SELECT * FROM venta_items WHERE venta_id = ?').all(req.params.id);
    for (const item of items) {
      const prod = db.prepare('SELECT * FROM productos WHERE id = ?').get(item.producto_id);
      if (prod) {
        const nuevo_stock = prod.stock + item.cantidad;
        db.prepare('UPDATE productos SET stock = ?, actualizado_en = CURRENT_TIMESTAMP WHERE id = ?')
          .run(nuevo_stock, item.producto_id);
        db.prepare('INSERT INTO movimientos (producto_id, tipo, cantidad, stock_anterior, stock_nuevo, nota, usuario_id) VALUES (?, ?, ?, ?, ?, ?, ?)')
          .run(item.producto_id, 'cancelacion', item.cantidad, prod.stock, nuevo_stock, `Cancelación venta ${venta.folio}`, req.user.id);
      }
    }
